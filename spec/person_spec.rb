...
describe 'Person' do
  ...
  describe '#new' do
    person = Person.new(age: 15, name: 'Susan', parent_permission: true)
    ...
    it 'has a set name Susan' do
      expect(person.name).to eq 'Susan'
    end

    it 'has a default name of Unknown' do
      expect(person.name).not_to eq 'Unknown'
    end
  ...
  describe 'Can use services' do
    ...
    context 'is adult' do
      it 'can use services' do
        person = Person.new(age: 18, name: 'Mithi', parent_permission: false)
        expect(person.can_use_services?).to be true
      end
    end
    # context 'use method to validate age' do
    ...
    context 'use method to correct name' do
      it 'correct name' do
        person = Person.new(age: 18, name: 'Thisus averylongname', parent_permission: false)
        expect(person.validate_name).to eq('Thisus ave')
      end
    end
  end
